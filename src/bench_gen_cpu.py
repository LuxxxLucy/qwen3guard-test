...
Benchmarks one (runtime, precision, opt_level) cell across both input
templates. Backends: PyTorch / ONNX / ONNX-GenAI / OpenVINO / llama.cpp /
vLLM-CPU / CTranslate2 / MNN-LLM. Opt-level ladder (cumulative):
  L0 (--unoptimized)     — model-card decode loop, 32 tokens.
  L1 (default)           — forced-prefix; one forward, read 3 verdict logits.
  L2 (--last-pos-logits) — L1 + project only the last position to vocab.
  L3 (--kv-cache)        — L2 + precompute the shared prefix KV once per call.

Examples:
  uv run python src/bench_gen_cpu.py --runtime pytorch
  uv run python src/bench_gen_cpu.py --runtime onnx --precision int8 \\
      --artifact var/models/onnx/Qwen3Guard-Gen-0.6B/int8
"""
...
def run_cell(backend, samples: list[list[int]], args, length: int | None,
    ...
    lat = warmup_and_measure(backend.predict, samples, args.n_warmup)
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", default="var/results")
