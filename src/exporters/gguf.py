...
Shallow-clones llama.cpp into var/vendor/, runs its convert_hf_to_gguf.py to
emit f32 / f16 / q8_0 directly (no C++ build needed). Output:
  var/models/gguf/<model_basename>.<quant>.gguf
"""
...
VENDOR = Path("var/vendor/llama.cpp")
OUT_DIR = Path("var/models/gguf")
...
def main() -> int:
    ...
    ap.add_argument("--quants", nargs="+", default=list(QUANTS), choices=QUANTS)
    args = ap.parse_args()
