...
Exit non-zero on drift. Run once per (runtime, precision, opt_level) row of the bench matrix.
"""
...
def _cache_path(model_id: str, samples: list[list[int]], vids: list[int],
    ...
    return Path("var/results/.cache") / f"{kind}_{key}.json"
...
def reference_logits(model_id: str, samples: list[list[int]],
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_id, torch_dtype=torch.float32).to("cpu").eval()
    out = []
    with torch.no_grad():
        for ids in samples:
...
def reference_l0_verdicts(model_id: str, plain_samples: list[list[int]],
    ...
    if cache.exists():
        return json.loads(cache.read_text())
    from backends import PyTorchCPUBackend
...
def main() -> int:
    ...
    ap.add_argument("--rust-inputs", default="src/backends/rust/bench_inputs.json",
    ...
    strict = precision.startswith("fp32")
