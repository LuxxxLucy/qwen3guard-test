...
class CTranslate2CPUBackend(GenBackend):
    """CTranslate2 CPU backend. Single whole-runtime row; the converter
    bakes L2 lastpos via `forward_batch`'s last-position slice and L1 via
    one forward over the forced prefix."""
    runtime = "ctranslate2"
    SUPPORTS_L0 = True
    ...
    def load(self, model_id: str, artifact: str | None, max_seq_len: int) -> None:
        ...
        if not (adir / "model.bin").exists():
            raise SystemExit(
                ...
                f"(run scripts/export_gen_ctranslate2.py first).")
